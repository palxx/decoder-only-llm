...
def xz_files_in_dir(directory):
    ...
    for folder in os.listdir(directory):
        ...
        folder = os.path.join(folder, 'openwebtext')
        print(folder)
        for filename in os.listdir(folder):
            print(filename)
            print(os.path.isfile(os.path.join(folder, filename)))
        #res = [filename for filename in os.listdir(folder) if filename.endswith(".xz") and os.path.isfile(os.path.join(folder, filename))]
        print(res)
...
folder_path = r"C:\Users\patron\Documents\research\ffc\subsets"
...
split_index = int(total_files * 0.5)  # 90% for training
